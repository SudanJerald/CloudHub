...
// Delete user
app.delete("/make-server-0b71ff0c/admin/delete-user", async (c) => {
  try {
    const { email } = await c.req.json();
...
app.get("/make-server-0b71ff0c/teacher/students", async (c) => {
  try {
    const allUsers = await db.getAllUsers();
    
    // Filter for approved students only
    const students = allUsers
      .filter(u => u.role === 'student' && u.account_status === 'approved')
      .map(u => ({
        id: u.email,
        name: u.full_name,
        email: u.email,
        ...
        yearSemester: u.year_semester || 'N/A',
        cgpa: u.cgpa || null
      }));
    ...
    for (const student of students) {
      const projects = await db.getProjects(student.email);
      const certificates = await db.getCertificates(student.email);
    ...
    const stats = {
      ...
      totalProjects,
      totalCertificates,
      recentActivities: 0 // Can be calculated from updated_at timestamps
    };
...
// Get student files
app.get("/make-server-0b71ff0c/teacher/student-files/:email", async (c) => {
  try {
    const email = c.req.param("email");
    
    // Fetch all data types for the student
    const [projects, certificates, notes, resumes] = await Promise.all([
      db.getProjects(email),
      db.getCertificates(email),
      db.getNotes(email),
      db.getResumes(email)
    ...
    // Return full data with all fields intact
    return c.json({
      projects: projects || [],
      certificates: certificates || [],
      notes: notes || [],
    ...
    return c.json({ error: "Failed to fetch student files" }, 500);
  }
});
